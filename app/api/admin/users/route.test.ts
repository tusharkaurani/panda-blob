...
describe("GET /api/admin/users", () => {
  ...
  it("derives blob_count as 0 for a user with no blobs", async () => {
    mocks.fake.__state.api_users.push(makeApiUser());
    const res = await GET(makeRequest("http://localhost/api/admin/users"));
  ...
  it("derives blob_count correctly for a user with several blobs", async () => {
    ...
    mocks.fake.__state.blobs.push(makeBlob({ owner_id: user.id }), makeBlob({ owner_id: user.id }), makeBlob({ owner_id: user.id }));
  ...
  it("filters by name search", async () => {
    mocks.fake.__state.api_users.push(makeApiUser({ name: "project-foo" }), makeApiUser({ name: "other" }));
    const res = await GET(makeRequest("http://localhost/api/admin/users?search=foo"));
    const body = await res.json();
...
describe("POST /api/admin/users", () => {
  it("passes through requireAdmin's error unchanged", async () => {
    ...
    const res = await POST(makeRequest("http://localhost/api/admin/users", { method: "POST", body: { name: "x" } }));
    expect(res.status).toBe(401);
  });

  it("returns 400 for a missing name", async () => {
    const res = await POST(makeRequest("http://localhost/api/admin/users", { method: "POST", body: {} }));
    expect(res.status).toBe(400);
  });

  it("returns 400 for a whitespace-only name", async () => {
    const res = await POST(
      makeRequest("http://localhost/api/admin/users", { method: "POST", body: { name: "   " } })
    );
    expect(res.status).toBe(400);
  });
  ...
  it("returns 400 for malformed JSON", async () => {
    const res = await POST(
      makeRequest("http://localhost/api/admin/users", { method: "POST", body: "{not json" })
    );
    expect(res.status).toBe(400);
  });
  ...
  it("creates a user with a generated pb_-prefixed access key, returns 201", async () => {
    const res = await POST(
      makeRequest("http://localhost/api/admin/users", { method: "POST", body: { name: "project-foo" } })
    );
  ...
  it("returns 500 when the insert fails", async () => {
    mocks.fake.__injectError("api_users", "insert");
    const res = await POST(
      makeRequest("http://localhost/api/admin/users", { method: "POST", body: { name: "x" } })
    );
    expect(res.status).toBe(500);
  });
});
